...
def _make_batch(batch: int, length: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    return torch.tensor(y0), torch.tensor(y1)
...
def _stream_inference(
    ...
    for start in range(0, length, chunk_size):
        ...
        outputs.append(streamer.process(y0_chunk, y1_chunk))
...
def test_streaming_matches_offline() -> None:
    ...
    y0, y1 = _make_batch(batch, length)
    model = MinimalGSENet()
...
def test_streaming_safe_prefix() -> None:
    ...
    y_stream = _stream_inference(streamer, y0, y1)
    streamer.reset()
    y_stream_changed = _stream_inference(streamer, y0_changed, y1_changed)
